...
function Game (name, minPlayers, maxPlayers, minTime, maxTime, lookOfGame, difficulty, pictureLink, webLink) { // construtor for Game objects
  ...
  this.pictureLink = pictureLink, // link to a picture of the game
  this.description = description, // decription of the game
  this.webLink = webLink; // link to boardgame geeks webpage for this game
...
var updatePassingArray = function (formPlayers, formTime, formLooks, library){ // create new function updatePassingArray, where:
  passingArray = []; // reset the passing array
  for (var gameIndex = 0; gameIndex < ourGameArray.length; gameIndex++){ // for every game...
    addIfPassing(library[gameIndex], formPlayers, formTime, formLooks); // add object to the passing array if it passes
  } // end for
...
var saveGames = function () {
  ...
  for (var k = 0; k < ourGameArray.length; k++) {
    localStorage['game ' + k + ' name'] = ourGameArray[k].name;
    localStorage['game ' + k + ' min number of players'] = ourGameArray[k].minPlayers;
    localStorage['game ' + k + ' max number of players'] = ourGameArray[k].maxPlayers;
    localStorage['game ' + k + ' minimum time'] = ourGameArray[k].minTime;
    localStorage['game ' + k + ' maximum time'] = ourGameArray[k].maxTime;
    localStorage['game ' + k + ' looks'] = ourGameArray[k].looksGood;
    localStorage['game ' + k + ' difficulty'] = ourGameArray[k].difficulty;
    localStorage.numberOfGames++;}
};

var loadGames = function () {
  for (var l = 0; l < localStorage.numberOfGames; l++) { // for every game in local storage
    ourGameArray.push( // add to the game array
...
function gameInput (event) { // create new function gameInput, where:
  ...
  ourGameArray.push ( // add to the gameArray
